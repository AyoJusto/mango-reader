...
internal fun decodeHtmlEntities(input: String): String {
    var result = input
        ...
        .replace("&quot;", "\"")
        .replace("&apos;", "'")
    result = numericHexEntity.replace(result) { m -> m.groupValues[1].toInt(16).toChar().toString() }
    result = numericDecEntity.replace(result) { m -> m.groupValues[1].toInt().toChar().toString() }
